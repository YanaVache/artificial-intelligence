...
class CleverPlayer:
    ...
    def __alphaBetaSearch(self, d, alpha = -1000000, beta = 1000000, toReturn = False):
        if d == 0:
            return self.__h()
        if self.board.whiteTurn == self.color: #szukamy najlepszej opcji dla siebie
            ...
            for (r, c) in self.board.getAllPossibilitiesToMove():
                ...
                self.board.addDisk(r, c)
                ret = self.__alphaBetaSearch(d-1, alpha, beta)
...
if __name__ == '__main__':
    start = time()
    simulation(CleverPlayer, RandomPlayer, 500)
    print(f"Czas trwania - {round(time() - start, 2)}s")